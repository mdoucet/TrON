...
def expt_from_json_file_v08(
    model_expt_json_file,
    q=None,
    q_resolution=0.025,
    probe=None,
    model_err_json_file=None,
    prior_scale=1,
    set_ranges=False,
):
    """
...
def expt_from_json(
    ...
    if model_err_json:
        if model_expt_json["probe"]["intensity"]["name"] in model_err_json:
            intensity_std = model_err_json[
                model_expt_json["probe"]["intensity"]["name"]
            ]["std"]
        if model_expt_json["probe"]["background"]["name"] in model_err_json:
            background_std = model_err_json[
                model_expt_json["probe"]["background"]["name"]
            ]["std"]
